...
<nav class="bg-white w-full lg:pr-20 py-5">
                ...
                @foreach ($menuLinks as $link)
                    @if($link->page_id)
                        <a href="{{ route('pages.show', $link->slug) }}"
                            class="text-sm text-gray-700 {{ request()->is('pages/' . $link->slug) ? 'font-bold' : '' }}">
                            {{ strtoupper($link->title) }}
                        </a>
                    @else
                        <a href="{{ route('categories.show', $link->slug) }}"
                            class="text-sm text-gray-700 {{ request()->is('categories/' . $link->slug) ? 'font-bold' : '' }}">
                            {{ strtoupper($link->title) }}
                        </a>
                    @endif
                @endforeach
    ...
    <span class="block">ANDRÉ</span>
            ...
            <div id="desktop-links" class="desktop-only space-x-4 items-center mr-0 lg:mr-8">
                <a href="/stills" class="text-sm text-gray-700 {{ request()->is('stills') ? 'font-bold' : '' }}">
                    STILLS
                </a>
                <a href="/contact" class="text-sm text-gray-700 {{ request()->is('contact') ? 'font-bold' : '' }}">
                    CONTACT
                </a>
            </div>
            ...
            <div class="flex flex-col h-full">
                ...
                <div class="flex-1 px-4 py-6 space-y-6">
                    @foreach ($menuLinks as $link)
                        @if($link->page_id)
                            <a href="{{ route('pages.show', $link->slug) }}"
                                class="block text-lg text-gray-700 py-2 border-b border-gray-100 {{ request()->is('pages/' . $link->slug) ? 'font-bold' : '' }}">
                                {{ strtoupper($link->title) }}
                            </a>
                        @else
                            <a href="{{ route('categories.show', $link->slug) }}"
                                class="block text-lg text-gray-700 py-2 border-b border-gray-100 {{ request()->is('categories/' . $link->slug) ? 'font-bold' : '' }}">
                                {{ strtoupper($link->title) }}
                            </a>
                        @endif
                    @endforeach

                    <a href="/stills" class="block text-lg text-gray-700 py-2 border-b border-gray-100">
                        STILLS
                    </a>
                    <a href="/contact" class="block text-lg text-gray-700 py-2 border-b border-gray-100">
                        CONTACT
                    </a>
                </div>
            </div>
